...
def test_renewable_generator_count(example_system, example_reeds_config) -> None:
    ...
    defaults = example_reeds_config.load_defaults()
...
def test_load_time_series_values(region_name, example_data_store, example_system) -> None:
    """Test that time series values match DataStore data for each region."""

    load_profiles = example_data_store.read_data("load_profiles").collect()
